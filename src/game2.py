...
class Game:
    def __init__(
        ...
        tracker_cls,
        visualizer_cls,
        camera_source=None,          # ✅ int (0) o str ("http://...") | si None usa camera_index
        camera_index: int = 0,       # ✅ compatibilidad con código antiguo
        use_dshow: bool = True,
        ...
        undistort_alpha: float = 0.0,            # 0.0 recorta más, 1.0 conserva más FOV (con bordes)
        undistort_crop: bool = False,            # recomiendo False para no cambiar el tamaño del frame
    ):
        self.config = config
        ...
        if camera_source is None:
            camera_source = camera_index  # si no pasas URL, usa el índice

        if isinstance(camera_source, int):
            api = cv2.CAP_DSHOW if use_dshow else 0
            self.cap = cv2.VideoCapture(camera_source, api)
        else:
            # URL (IP cam). CAP_FFMPEG suele ir mejor si tu OpenCV lo trae.
        ...
        # Opcional: reduce latencia (no siempre funciona según backend)
        try:
        ...
        self._undistort_enabled = calibration_npz is not None
        ...
        if calibration_npz is not None:
            try:
                ...
                # No rompemos el juego si la calibración no se puede cargar
                self.undistorter = None
    ...
    def _read_and_render(self, overlay_text=None):
        ...
        if self.undistorter is not None:
            try:
                ...
                # No rompemos el juego si undistort falla un frame
    ...
    def _snapshot_with_masks(self, overlay_text="SHOT"):
        ...
        # ✅ Apply undistort here too (so exported snapshot matches tracker space)
        if self.undistorter is not None:
            try:
                frame = self.undistorter(frame)
            except Exception as e:
    ...
    def _release(self):
        ...
        if self.save_results:
            ...
            try:
                ...
                if _DASH_AVAILABLE and ResultsDashboard is not None:
                    dashboard = ResultsDashboard(out_dir, port=5000, open_browser=True)
                    dashboard.run(blocking=True)
                else:
                    print("Dashboard not available; skipping launch.")
